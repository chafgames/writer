...
class DrawDungeon(Effect):
    ...
    def __init__(self, screen, game_state):
        super(DrawDungeon, self).__init__(screen)
        self._state = game_state
#        self._x = self._screen.width - 2 * (self._size + 1)
#        self._y = self._screen.height - (self._size + 1)
#        self._x = self._screen.width // 2
#        self._y = self._screen.height // 2
    ...
    def _update(self, _):
        ...
        for mx in range(0, self._screen.width, 2):
            for my in range(self._screen.height):
                px = self._state.map_x
                #     colour = Screen.COLOUR_GREEN
                # else:
                #     colour = Screen.COLOUR_BLACK
                colour = Screen.COLOUR_BLACK
                dbg_vals = f"mx:{mx} my:{my} gsx:{game_state.x} gsy:{game_state.y} pang:{game_state.player_angle} smx:{self._state.map_x}         "# px:{px} py:{py}                "
                logging.info(dbg_vals)
#                self._screen.print_at(dbg_vals, 2, (self._screen.height) // 2, Screen.COLOUR_WHITE, bg=0)
                self._screen.print_at("  ", mx, my, colour, bg=colour)
        ...
        for a, b, direction in self._DIRECTIONS:
            if a < self._state.player_angle <= b:
                ...
                break
        self._screen.print_at(
            text, self._x + self._screen.width // 2 * 2, self._y + self._screen.height // 2, Screen.COLOUR_GREEN)
...
#        self._screen.clear_buffer(Screen.COLOUR_BLACK, Screen.A_NORMAL, Screen.COLOUR_BLACK)
#
#        colour, attr, bg = self._colours[min(len(self._colours) - 1, int(3 * 1))]
#        self._screen.print_at(
#            "TEXT" * 2, 2, (self._screen.height) // 2,
...
class GameController(Scene):
    ...
    def __init__(self, screen, game_state):
        ...
        effects = [
            DrawDungeon(screen, self._state),
            self._mini_map
        ]
